...
ruby_block "Wait for #{node_count} node to have CoreOS installed..." do
  block do
    puts ''
    node.run_state['logs'] = {}
    loop do
      if Hanlon::Api::ActiveModel.list.length >= node_count
        ...
        sleeptime = 30
        Chef::Log.warn("Waiting #{sleeptime} seconds for (#{Hanlon::Api::ActiveModel.list.length}/#{node_count}) nodes to start Installing CoreOS ")
        sleep sleeptime
      end
    end
  end
